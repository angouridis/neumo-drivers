...
static int dvb_usbv2_download_firmware(struct neumo_dvb_usb_device *d,
		const char *name)
{
	int ret;
...
static int dvb_usbv2_i2c_init(struct neumo_dvb_usb_device *d)
{
	int ret;
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
...
static int dvb_usbv2_i2c_exit(struct neumo_dvb_usb_device *d)
{
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
...
{
	struct neumo_dvb_usb_device *d = container_of(work,
			struct neumo_dvb_usb_device, rc_query_work.work);
	int ret;
...
static int dvb_usbv2_remote_init(struct neumo_dvb_usb_device *d)
{
	int ret;
	struct rc_dev *dev;
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
...
static int dvb_usbv2_remote_exit(struct neumo_dvb_usb_device *d)
{
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
		...
		size_t len)
{
	...
	neumo_dvb_dmx_swfilter(&adap->demux, buf, len);
}
...
{
	struct neumo_dvb_usb_adapter *adap = stream->user_priv;
	neumo_dvb_dmx_swfilter_204(&adap->demux, buf, len);
}
...
{
	struct neumo_dvb_usb_adapter *adap = stream->user_priv;
	neumo_dvb_dmx_swfilter_raw(&adap->demux, buf, len);
}

static int dvb_usbv2_adapter_stream_init(struct neumo_dvb_usb_adapter *adap)
{
...
static int dvb_usbv2_adapter_stream_exit(struct neumo_dvb_usb_adapter *adap)
{
	dev_dbg(&adap_to_d(adap)->udev->dev, "%s: adap=%d\n", __func__,
			adap->id);
...
static int dvb_usbv2_media_device_init(struct neumo_dvb_usb_adapter *adap)
{
...
static int dvb_usbv2_media_device_register(struct neumo_dvb_usb_adapter *adap)
{
#ifdef CONFIG_MEDIA_CONTROLLER_DVB
...
static void dvb_usbv2_media_device_unregister(struct neumo_dvb_usb_adapter *adap)
{
#ifdef CONFIG_MEDIA_CONTROLLER_DVB
...
static int dvb_usbv2_adapter_dvb_init(struct neumo_dvb_usb_adapter *adap)
{
	int ret;
	struct neumo_dvb_usb_device *d = adap_to_d(adap);
	...
	ret = neumo_dvb_dmx_init(&adap->demux);
	if (ret < 0) {
		dev_err(&d->udev->dev, "%s: neumo_dvb_dmx_init() failed=%d\n",
				KBUILD_MODNAME, ret);
		goto err_dvb_dmx_init;
	}
	...
	ret = neumo_dvb_dmxdev_init(&adap->dmxdev, &adap->dvb_adap);
	if (ret < 0) {
		dev_err(&d->udev->dev, "%s: neumo_dvb_dmxdev_init() failed=%d\n",
				KBUILD_MODNAME, ret);
...
static int dvb_usbv2_adapter_dvb_exit(struct neumo_dvb_usb_adapter *adap)
{
	dev_dbg(&adap_to_d(adap)->udev->dev, "%s: adap=%d\n", __func__,
			adap->id);
...
static int dvb_usbv2_device_power_ctrl(struct neumo_dvb_usb_device *d, int onoff)
{
	int ret;
...
static int dvb_usb_fe_init(struct neumo_dvb_frontend *fe)
{
	int ret;
...
static int neumo_dvb_usbv2_adapter_frontend_init(struct neumo_dvb_usb_adapter *adap)
{
	...
	for (i = 0; i < MAX_NO_OF_FE_PER_ADAP && adap->fe[i]; i++) {
		...
		ret = neumo_dvb_register_frontend(&adap->dvb_adap, adap->fe[i]);
		if (ret < 0) {
			dev_err(&d->udev->dev,
...
err_dvb_unregister_frontend:
	for (i = count_registered - 1; i >= 0; i--)
		neumo_dvb_unregister_frontend(adap->fe[i]);

err_dvb_frontend_detach:
	for (i = MAX_NO_OF_FE_PER_ADAP - 1; i >= 0; i--) {
		if (adap->fe[i]) {
			neumo_dvb_frontend_detach(adap->fe[i]);
			adap->fe[i] = NULL;
		}
	}
...
static int dvb_usbv2_adapter_frontend_exit(struct neumo_dvb_usb_adapter *adap)
{
	int ret, i;
	struct neumo_dvb_usb_device *d = adap_to_d(adap);
...
static int dvb_usbv2_adapter_init(struct neumo_dvb_usb_device *d)
{
	...
	for (i = 0; i < adapter_count; i++) {
		...
		ret = neumo_dvb_usbv2_adapter_frontend_init(adap);
		if (ret)
			goto err;
...
static int dvb_usbv2_adapter_exit(struct neumo_dvb_usb_device *d)
{
	int i;
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
...
static int dvb_usbv2_exit(struct neumo_dvb_usb_device *d)
{
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
...
static int dvb_usbv2_init(struct neumo_dvb_usb_device *d)
{
	int ret;
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
...
int neumo_dvb_usbv2_probe(struct usb_interface *intf,
													const struct usb_device_id *id)
{
	int ret;
	struct neumo_dvb_usb_device *d;
	struct usb_device *udev = interface_to_usbdev(intf);
	struct neumo_dvb_usb_driver_info *driver_info =
			(struct neumo_dvb_usb_driver_info *) id->driver_info;
	dprintk("Called\n");
	...
	d = kzalloc(sizeof(struct neumo_dvb_usb_device), GFP_KERNEL);
	if (!d) {
...
EXPORT_SYMBOL(neumo_dvb_usbv2_probe);

void neumo_dvb_usbv2_disconnect(struct usb_interface *intf)
{
...
EXPORT_SYMBOL(neumo_dvb_usbv2_disconnect);

int neumo_dvb_usbv2_suspend(struct usb_interface *intf, pm_message_t msg)
{
	struct neumo_dvb_usb_device *d = usb_get_intfdata(intf);
	...
	for (i = MAX_NO_OF_ADAPTER_PER_DEVICE - 1; i >= 0; i--) {
		...
		if (d->adapter[i].dvb_adap.priv && active_fe != -1) {
			...
			ret = neumo_dvb_frontend_suspend(fe);
		}
	}

	return ret;
}
EXPORT_SYMBOL(neumo_dvb_usbv2_suspend);

static int dvb_usbv2_resume_common(struct neumo_dvb_usb_device *d)
{
	int ret = 0, i, active_fe;
	struct neumo_dvb_frontend *fe;
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
	...
	for (i = 0; i < MAX_NO_OF_ADAPTER_PER_DEVICE; i++) {
		...
		if (d->adapter[i].dvb_adap.priv && active_fe != -1) {
			...
			ret = neumo_dvb_frontend_resume(fe);
...
int neumo_dvb_usbv2_resume(struct usb_interface *intf)
{
	struct neumo_dvb_usb_device *d = usb_get_intfdata(intf);
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
...
EXPORT_SYMBOL(neumo_dvb_usbv2_resume);

int neumo_dvb_usbv2_reset_resume(struct usb_interface *intf)
{
	struct neumo_dvb_usb_device *d = usb_get_intfdata(intf);
	int ret;
	dev_dbg(&d->udev->dev, "%s:\n", __func__);
...
EXPORT_SYMBOL(neumo_dvb_usbv2_reset_resume);
