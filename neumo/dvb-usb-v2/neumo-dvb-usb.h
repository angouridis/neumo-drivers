 ...
 * d = pointer to device (struct neumo_dvb_usb_device *)
 * adap = pointer to adapter (struct neumo_dvb_usb_adapter *)
 * fe = pointer to frontend (struct neumo_dvb_frontend *)
 *
 * Use macros defined in that file to resolve needed pointers.
 */

/* helper macros for every DVB USB driver use */
#define adap_to_d(adap) (container_of(adap, struct neumo_dvb_usb_device, \
		adapter[adap->id]))
#define adap_to_priv(adap) (adap_to_d(adap)->priv)
#define fe_to_adap(fe) ((struct neumo_dvb_usb_adapter *) ((fe)->dvb->priv))
...
#define DVB_USB_DEVICE(vend, prod, props_, name_, rc) \
	...
	.idVendor = (vend), \
	.idProduct = (prod), \
	.driver_info = (kernel_ulong_t) &((const struct neumo_dvb_usb_driver_info) { \
		.props = (props_), \
		.name = (name_), \
		.rc_map = (rc), \
	})
...
/**
 * struct neumo_dvb_usb_driver_info - structure for carrying all needed data from the
 ...
 */
struct neumo_dvb_usb_driver_info {
	const char *name;
	const char *rc_map;
...
/**
 * struct neumo_dvb_usb_rc - structure for remote controller configuration
 ...
 */
struct neumo_dvb_usb_rc {
	const char *map_name;
	u64 allowed_protos;
	int (*change_protocol)(struct rc_dev *dev, u64 *rc_proto);
	int (*query) (struct neumo_dvb_usb_device *d);
...
/**
 * struct neumo_dvb_usb_adapter_properties - properties of dvb usb device adapter
...
#define MAX_NO_OF_ADAPTER_PER_DEVICE 2
	...
	int (*probe)(struct neumo_dvb_usb_device *);
	void (*disconnect)(struct neumo_dvb_usb_device *);
#define WARM                  0
#define COLD                  1
	int (*identify_state) (struct neumo_dvb_usb_device *, const char **);
	const char *firmware;
#define RECONNECTS_USB        1
	int (*download_firmware) (struct neumo_dvb_usb_device *,
			const struct firmware *);

	const struct i2c_algorithm *i2c_algo;

	unsigned int num_adapters;
	int (*get_adapter_count) (struct neumo_dvb_usb_device *);
	...
	int (*read_config) (struct neumo_dvb_usb_device *d);
	int (*read_mac_address) (struct neumo_dvb_usb_adapter *, u8 []);
	int (*frontend_attach) (struct neumo_dvb_usb_adapter *);
	int (*frontend_detach)(struct neumo_dvb_usb_adapter *);
	int (*tuner_attach) (struct neumo_dvb_usb_adapter *);
	int (*tuner_detach)(struct neumo_dvb_usb_adapter *);
	int (*frontend_ctrl) (struct neumo_dvb_frontend *, int);
	int (*streaming_ctrl) (struct neumo_dvb_frontend *, int);
	int (*init) (struct neumo_dvb_usb_device *);
	void (*exit) (struct neumo_dvb_usb_device *);
	int (*get_rc_config) (struct neumo_dvb_usb_device *, struct neumo_dvb_usb_rc *);
#define DVB_USB_FE_TS_TYPE_188        0
#define DVB_USB_FE_TS_TYPE_204        1
#define DVB_USB_FE_TS_TYPE_RAW        2
	int (*get_stream_config) (struct neumo_dvb_frontend *,  u8 *,
			struct usb_data_stream_properties *);
};
...
/**
 * struct neumo_dvb_usb_adapter - dvb adapter object on dvb usb device
 ...
 */
struct neumo_dvb_usb_adapter {
	const struct neumo_dvb_usb_adapter_properties *props;
...
#define ADAP_STREAMING           2
	...
	struct neumo_dvb_frontend *fe[MAX_NO_OF_FE_PER_ADAP];
	int (*fe_init[MAX_NO_OF_FE_PER_ADAP]) (struct neumo_dvb_frontend *);
	int (*fe_sleep[MAX_NO_OF_FE_PER_ADAP]) (struct neumo_dvb_frontend *);
};

/**
 * struct neumo_dvb_usb_device - dvb usb device object
...
extern int neumo_dvb_usbv2_probe(struct usb_interface *,
		const struct usb_device_id *);
extern void neumo_dvb_usbv2_disconnect(struct usb_interface *);
extern int neumo_dvb_usbv2_suspend(struct usb_interface *, pm_message_t);
extern int neumo_dvb_usbv2_resume(struct usb_interface *);
extern int neumo_dvb_usbv2_reset_resume(struct usb_interface *);

/* the generic read/write method for device control */
extern int neumo_dvb_usbv2_generic_rw(struct neumo_dvb_usb_device *, u8 *, u16, u8 *, u16);
extern int neumo_dvb_usbv2_generic_write(struct neumo_dvb_usb_device *, u8 *, u16);
/* caller must hold lock when locked versions are called */
extern int neumo_dvb_usbv2_generic_rw_locked(struct neumo_dvb_usb_device *,
		u8 *, u16, u8 *, u16);
extern int neumo_dvb_usbv2_generic_write_locked(struct neumo_dvb_usb_device *, u8 *, u16);
