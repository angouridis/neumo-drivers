...
static int rtl28xxu_ctrl_msg(struct neumo_dvb_usb_device *d, struct rtl28xxu_req *req)
{
...
static int rtl28xxu_wr_regs(struct neumo_dvb_usb_device *d, u16 reg, u8 *val, int len)
{
...
static int rtl28xxu_rd_regs(struct neumo_dvb_usb_device *d, u16 reg, u8 *val, int len)
{
	struct rtl28xxu_req req;
...
static int rtl28xxu_wr_reg(struct neumo_dvb_usb_device *d, u16 reg, u8 val)
{
	return rtl28xxu_wr_regs(d, reg, &val, 1);
}

static int rtl28xxu_rd_reg(struct neumo_dvb_usb_device *d, u16 reg, u8 *val)
{
	return rtl28xxu_rd_regs(d, reg, val, 1);
}

static int rtl28xxu_wr_reg_mask(struct neumo_dvb_usb_device *d, u16 reg, u8 val,
		u8 mask)
{
	int ret;
	u8 tmp;
...
static int rtl2831u_read_config(struct neumo_dvb_usb_device *d)
{
...
static int rtl2832u_read_config(struct neumo_dvb_usb_device *d)
{
	struct rtl28xxu_dev *dev = d_to_priv(d);
	int ret;
	u8 buf[2];
...
static int rtl28xxu_read_config(struct neumo_dvb_usb_device *d)
{
	struct rtl28xxu_dev *dev = d_to_priv(d);
...
static int rtl28xxu_identify_state(struct neumo_dvb_usb_device *d, const char **name)
{
	struct rtl28xxu_dev *dev = d_to_priv(d);
	int ret;
...
static int rtl2831u_frontend_attach(struct neumo_dvb_usb_adapter *adap)
{
	struct neumo_dvb_usb_device *d = adap_to_d(adap);
	struct rtl28xxu_dev *dev = d_to_priv(d);
...
static int rtl2832u_fc0012_tuner_callback(struct neumo_dvb_usb_device *d,
		int cmd, int arg)
{
	int ret;
	u8 val;
...
static int rtl2832u_tua9001_tuner_callback(struct neumo_dvb_usb_device *d,
		int cmd, int arg)
{
	int ret;
	u8 val;
...
static int rtl2832u_frontend_attach(struct neumo_dvb_usb_adapter *adap)
{
	struct neumo_dvb_usb_device *d = adap_to_d(adap);
	struct rtl28xxu_dev *dev = d_to_priv(d);
...
static int rtl28xxu_frontend_attach(struct neumo_dvb_usb_adapter *adap)
{
	...
	dprintk("Called\n");
	if (dev->chip_id == CHIP_ID_RTL2831U)
		ret = rtl2831u_frontend_attach(adap);
	else
		ret = rtl2832u_frontend_attach(adap);
	dprintk("Called\n");
	for(int i=0; i < MAX_NO_OF_ADAPTER_PER_DEVICE; ++i)  {
		struct neumo_dvb_frontend* fe =	adap->fe[i];
		dprintk("Called fe=%p\n", fe);
		if(fe) {
			snprintf(fe->ops.info.card_address, sizeof(fe->ops.info.card_address),
					 "usb%s", dev_name(&d->udev->dev));
			dprintk("serial=%s\n",  d->udev->serial);
			if(d->udev->serial) {
				fe->ops.info.card_mac_address = usb_serial_to_mac_address(d->udev->serial);
				fe->ops.info.adapter_mac_address = fe->ops.info.card_mac_address +i;
				dprintk("SET card address: %s mac=%llx adapter_max=%llx", adap->fe[i]->ops.info.card_address,
								fe->ops.info.card_mac_address, fe->ops.info.adapter_mac_address
								);
			}
		}
	}
...
static int rtl28xxu_frontend_detach(struct neumo_dvb_usb_adapter *adap)
{
	struct neumo_dvb_usb_device *d = adap_to_d(adap);
	struct rtl28xxu_dev *dev = d_to_priv(d);
	struct i2c_client *client;
...
static int rtl2831u_tuner_attach(struct neumo_dvb_usb_adapter *adap)
{
	int ret;
	struct neumo_dvb_usb_device *d = adap_to_d(adap);
	struct rtl28xxu_dev *dev = d_to_priv(d);
...
static int rtl2832u_tuner_attach(struct neumo_dvb_usb_adapter *adap)
{
	int ret;
	struct neumo_dvb_usb_device *d = adap_to_d(adap);
	struct rtl28xxu_dev *dev = d_to_priv(d);
	struct neumo_dvb_frontend *fe = NULL;
	struct i2c_board_info info;
	struct i2c_client *client;
	...
	case TUNER_RTL2832_R820T:
		dprintk("Called rf_in=%d\n", rf_in);
		fe = dvb_attach(r820t_attach, adap->fe[0],
										dev->demod_i2c_adapter,
										&rtl2832u_r820t_config, rf_in);
...
static int rtl28xxu_tuner_attach(struct neumo_dvb_usb_adapter *adap)
{
	struct rtl28xxu_dev *dev = adap_to_priv(adap);
...
static int rtl28xxu_tuner_detach(struct neumo_dvb_usb_adapter *adap)
{
	struct neumo_dvb_usb_device *d = adap_to_d(adap);
	struct rtl28xxu_dev *dev = d_to_priv(d);
	struct i2c_client *client;
...
static int rtl28xxu_init(struct neumo_dvb_usb_device *d)
{
	int ret;
	u8 val;
...
static int rtl2831u_power_ctrl(struct neumo_dvb_usb_device *d, int onoff)
{
	int ret;
...
static int rtl2832u_power_ctrl(struct neumo_dvb_usb_device *d, int onoff)
{
	int ret;
...
static int rtl28xxu_power_ctrl(struct neumo_dvb_usb_device *d, int onoff)
{
	struct rtl28xxu_dev *dev = d_to_priv(d);
...
static int rtl28xxu_frontend_ctrl(struct neumo_dvb_frontend *fe, int onoff)
{
	struct neumo_dvb_usb_device *d = fe_to_d(fe);
...
static int rtl2831u_rc_query(struct neumo_dvb_usb_device *d)
{
	int ret, i;
	struct rtl28xxu_dev *dev = d->priv;
	u8 buf[5];
...
static int rtl2831u_get_rc_config(struct neumo_dvb_usb_device *d,
		struct neumo_dvb_usb_rc *rc)
{
...
static int rtl2832u_rc_query(struct neumo_dvb_usb_device *d)
{
	int ret, i, len;
	struct rtl28xxu_dev *dev = d->priv;
...
static int rtl2832u_get_rc_config(struct neumo_dvb_usb_device *d,
		struct neumo_dvb_usb_rc *rc)
{
...
static int rtl28xxu_get_rc_config(struct neumo_dvb_usb_device *d,
		struct neumo_dvb_usb_rc *rc)
{
	struct rtl28xxu_dev *dev = d_to_priv(d);
...
static int rtl28xxu_pid_filter_ctrl(struct neumo_dvb_usb_adapter *adap, int onoff)
{
	struct rtl28xxu_dev *dev = adap_to_priv(adap);
...
static int rtl28xxu_pid_filter(struct neumo_dvb_usb_adapter *adap, int index,
			       u16 pid, int onoff)
{
	struct rtl28xxu_dev *dev = adap_to_priv(adap);
...
static const struct neumo_dvb_usb_device_properties rtl28xxu_props = {
...
static struct usb_driver rtl28xxu_usb_driver = {
	...
	.probe = neumo_dvb_usbv2_probe,
	.disconnect = neumo_dvb_usbv2_disconnect,
	.suspend = neumo_dvb_usbv2_suspend,
	.resume = neumo_dvb_usbv2_resume,
	.reset_resume = neumo_dvb_usbv2_reset_resume,
	.no_dynamic_id = 1,
	.soft_unbind = 1,
};
