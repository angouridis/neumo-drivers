...
static int gx1503_init(struct neumo_dvb_frontend *fe)
{
...
static int gx1503_set_frontend(struct neumo_dvb_frontend *fe)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct gx1503_dev *dev = i2c_get_clientdata(client);
...
static int gx1503_read_status(struct neumo_dvb_frontend *fe, enum fe_status *status)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct gx1503_dev *dev = i2c_get_clientdata(client);
	int ret,temp;
...
static const struct neumo_dvb_frontend_ops gx1503_ops = {
		.delsys = {SYS_DVBT},
		.info = {
...
#endif
	/*create dvb frontend*/
	memcpy(&dev->fe.ops,&gx1503_ops,sizeof(struct neumo_dvb_frontend_ops));
