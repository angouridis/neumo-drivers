 ...
 * (c) 2020-2025 Deep Thought <deeptho@gmail.com>
...
s32 si2183_signal_power_dbm(struct neumo_dvb_frontend *fe)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct si2183_cmd cmd;
	u16 agc1, agc2;
...
int si2183_isi_scan(struct neumo_dvb_frontend* fe, int num_isi)
{
	struct i2c_client* client = fe->demodulator_priv;
	struct si2183_dev* state = i2c_get_clientdata(client);
	struct neumo_dtv_frontend_properties* p = &fe->dtv_property_cache;
...
int si2183_stop_task(struct neumo_dvb_frontend *fe)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct si2183_dev *state = i2c_get_clientdata(client);
...
int si2183_constellation_start(struct neumo_dvb_frontend *fe, struct dtv_fe_constellation* user, int max_num_samples)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct si2183_dev* state = i2c_get_clientdata(client);
	struct constellation_scan_state* cs = &state->constellation_scan_state;
...
int si2183_constellation_get(struct neumo_dvb_frontend *fe, struct dtv_fe_constellation* user)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct si2183_dev* state = i2c_get_clientdata(client);
	struct constellation_scan_state* cs = &state->constellation_scan_state;
...
static u8 Si2183_convert_to_byte (const u8* buffer, u8 shift, u8 mask) {
...
static int si2183_get_spectrum_scan_fft_one_band(struct neumo_dvb_frontend *fe, u32* freq, s32* rf_level,
																								 int max_fft_size, int tuned_freq);

int si2183_scan_sat_(struct neumo_dvb_frontend *fe, bool init,
 ...
 // while(1)
	 ...
	 if(bs->start_resume) {
		 ...
		 while (scan_status->buz == Si2183_SCAN_STATUS_RESPONSE_BUZ_BUSY) {
				 if ((count% 20==19) && (kthread_should_stop() || neumo_dvb_frontend_task_should_stop(fe))) {
					 goto _exit;
				 }
				 si2183_scan_status(client, Si2183_SCAN_STATUS_CMD_INTACK_OK, scan_status);
				 msleep(200);
...
int si2183_scan_sat(struct neumo_dvb_frontend *fe, bool init, unsigned int *delay, enum fe_status *status)
{
...
//Si2183_WRITE (front_end->demod, dcom_control_byte, (0x80000000 | (nb_word - 1)) );
	...
	for (w=0; w < 579/*nb_word*/ ; ++w) {
		if ((w% 20==19) &&  (kthread_should_stop() || neumo_dvb_frontend_task_should_stop(fe))) {
			dprintk("exiting on should stop");
			break;
		}
...
int si2183_spectrum_start(struct neumo_dvb_frontend *fe,
...
int si2183_spectrum_get(struct neumo_dvb_frontend *fe, struct dtv_fe_spectrum* user)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct si2183_dev *state = i2c_get_clientdata(client);
...
#if 0 //unused
static int si2183_rssi_adc(struct i2c_client *client, bool on_off, int *rssi)
{
	//unsigned int value;
	struct si2183_cmd cmd;
...
int blind_tune(struct neumo_dvb_frontend *fe)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct si2183_dev* state = i2c_get_clientdata(client);
	struct neumo_dtv_frontend_properties* p = &fe->dtv_property_cache;
