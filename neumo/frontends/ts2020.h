...
#define TS2020_CLK_OUT_ENABLED_XTALOUT 2
	 ...
	 */
	int (*get_agc_pwm)(struct neumo_dvb_frontend *fe, u8 *_agc_pwm);
};

/* Do not add new ts2020_attach() users! Use I2C bindings instead. */
#if IS_REACHABLE(CONFIG_DVB_TS2020)
extern struct neumo_dvb_frontend *ts2020_attach(
	struct neumo_dvb_frontend *fe,
	const struct ts2020_config *config,
	struct i2c_adapter *i2c);
#else
static inline struct neumo_dvb_frontend *ts2020_attach(
	struct neumo_dvb_frontend *fe,
	const struct ts2020_config *config,
	struct i2c_adapter *i2c)
{
