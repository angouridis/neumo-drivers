...
/* bus = 1   demod    */
/* bus = 2   tuner    */
struct i2c_adapter *tas2101_get_i2c_adapter(struct neumo_dvb_frontend *fe, int bus)
{
...
static int tas2101_stop_task(struct neumo_dvb_frontend *fe);
...
/*
	...
	unit: 0.001dB
 */
static int tas2101_signal_power_dbm(struct neumo_dvb_frontend *fe, long* val)
{
...
static inline int tas2101_narrow_band_signal_power_dbm(struct neumo_dvb_frontend *fe, long* val) {
	return tas2101_signal_power_dbm(fe, val);
}

static int tas2101_snr(struct neumo_dvb_frontend *fe, s32* snr)
{
	struct tas2101_priv *state = fe->demodulator_priv;
	u8 buf[2];
	u16 raw;
...
static inline int tas2101_get_modulation(struct neumo_dvb_frontend *fe)
{
	struct tas2101_priv *state = fe->demodulator_priv;
	u8 temp;
	int idx =0;
...
static inline int tas2101_delsys(struct neumo_dvb_frontend *fe, int* delsys)
{
	u8 temp;
	struct tas2101_priv *state = fe->demodulator_priv;
...
static void tas2101_spi_read(struct neumo_dvb_frontend *fe, struct ecp3_info *ecp3inf)
{
...
static void tas2101_spi_write(struct neumo_dvb_frontend *fe,struct ecp3_info *ecp3inf)
{
	struct tas2101_priv *priv = fe->demodulator_priv;
	struct i2c_adapter *adapter = priv->i2c;
...
static void tas2101_eeprom_read(struct neumo_dvb_frontend *fe, struct eeprom_info *eepinf)
{
	struct tas2101_priv *priv = fe->demodulator_priv;
	struct i2c_adapter *adapter = priv->i2c;
...
static void tas2101_eeprom_write(struct neumo_dvb_frontend *fe,struct eeprom_info *eepinf)
{
	struct tas2101_priv *priv = fe->demodulator_priv;
	struct i2c_adapter *adapter = priv->i2c;
...
struct neumo_dvb_frontend *tas2101_attach(const struct tas2101_config *cfg,
...
static int set_frontend(struct neumo_dvb_frontend *fe)
{
	bool need_retune;
...
static int tas2101_constellation_start(struct neumo_dvb_frontend *fe,
...
static enum neumo_dvbfe_search tas2101_search(struct neumo_dvb_frontend *fe)
{
	struct tas2101_priv *priv = fe->demodulator_priv;
...
static int tas2101_spectrum_start(struct neumo_dvb_frontend *fe,
																	 ...
																	 unsigned int *delay, enum fe_status *status)
{
	...
	*status =  FE_HAS_SIGNAL|FE_HAS_CARRIER|FE_HAS_VITERBI|FE_HAS_SYNC|
		FE_HAS_LOCK|FE_HAS_TIMING_LOCK;
	return 0;
}
...
static int tas2101_spectrum_get(struct neumo_dvb_frontend *fe, struct dtv_fe_spectrum* user)
{
	struct tas2101_priv *state = fe->demodulator_priv;
	int error=0;
...
#ifndef TAS2101_USE_I2C_MUX
static int tas2101_i2c_gate_ctrl(struct neumo_dvb_frontend* fe, int enable)
{
	struct tas2101_priv *priv = fe->demodulator_priv;
	int ret;
...
static int tas2101_constellation_get(struct neumo_dvb_frontend *fe, struct dtv_fe_constellation* user)
{
	struct tas2101_priv *state = fe->demodulator_priv;
	struct tas2101_constellation_scan_state* cs = &state->constellation_scan_state;
	int error = 0;
