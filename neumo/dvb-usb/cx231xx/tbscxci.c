...
static int tbscxci_i2c_read(struct tbscxci_state *state)
{
	int ret;
	u8 buf = 0;
...
static int tbscxci_i2c_write(struct tbscxci_state *state,
...
int tbscxci_init(struct cx231xx_dvb *adap, int nr)
{
	...
	ret = neumo_dvb_ca_en50221_init(&adap->adapter, &state->ca,
		/* flags */ 0, /* n_slots */ 1);
