...
static void tbs5990_lnb_power(struct neumo_dvb_frontend *fe,
...
static void tbs5990_lnb0_power(struct neumo_dvb_frontend *fe, int onoff)
{
	tbs5990_lnb_power(fe, 26, onoff);
}

static void tbs5990_lnb1_power(struct neumo_dvb_frontend *fe, int onoff)
{
...
static inline int dvb_isoc_copy(struct cx231xx *dev, struct urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		neumo_dvb_dmx_swfilter(&dev->dvb[0]->demux,
				 urb->transfer_buffer +
...
static inline int dvb_isoc_copy_ts2(struct cx231xx *dev, struct urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		neumo_dvb_dmx_swfilter(&dev->dvb[1]->demux,
				 urb->transfer_buffer +
				urb->iso_frame_desc[i].offset,
				urb->iso_frame_desc[i].actual_length);
	}
...
static int start_feed(struct neumo_dvb_demux_feed *feed)
{
...
static int stop_feed(struct neumo_dvb_demux_feed *feed)
{
	struct neumo_dvb_demux *demux = feed->demux;
	struct cx231xx_dvb *dvb = demux->priv;
	int err = 0;
...
/* ------------------------------------------------------------------ */
static int cx231xx_dvb_bus_ctrl(struct neumo_dvb_frontend *fe, int acquire)
{
	struct cx231xx *dev = fe->dvb->priv;
...
static int attach_xc5000(u8 addr, struct cx231xx *dev)
{
	...
	fe = dvb_attach(xc5000_attach, dev->dvb[0]->frontend[0], &cfg, 0);
	if (!fe) {
...
int cx231xx_set_analog_freq(struct cx231xx *dev, u32 freq)
{
	if ((dev->dvb[0] != NULL) && (dev->dvb[0]->frontend[0] != NULL)) {
		...
		struct neumo_dvb_tuner_ops *dops = &dev->dvb[0]->frontend[0]->ops.tuner_ops;
			...
			struct cx231xx *dev, struct device *device)
{
	...
	/* register frontend */
	result = neumo_dvb_register_frontend(&dvb->adapter, dvb->frontend[0]);
	if (result < 0) {
		dev_warn(dev->dev,
	...
	if (dvb->frontend[1]) {
		result = neumo_dvb_register_frontend(&dvb->adapter, dvb->frontend[1]);
		if (result < 0) {
			dev_warn(dev->dev,
	...
	result = neumo_dvb_dmx_init(&dvb->demux);
	if (result < 0) {
		dev_warn(dev->dev,
	...
	result = neumo_dvb_dmxdev_init(&dvb->dmxdev, &dvb->adapter);
	if (result < 0) {
		dev_warn(dev->dev,
	...
	memcpy(dvb->adapter.proposed_mac, &dvb->adapter_mac_address, 6);
...
fail_fe_hw:
	neumo_dvb_dmxdev_release(&dvb->dmxdev);
fail_dmxdev:
	neumo_dvb_dmx_release(&dvb->demux);
fail_dmx:
	if (dvb->frontend[1])
		neumo_dvb_unregister_frontend(dvb->frontend[1]);
	neumo_dvb_unregister_frontend(dvb->frontend[0]);
fail_frontend1:
	if (dvb->frontend[1])
		neumo_dvb_frontend_detach(dvb->frontend[1]);
fail_frontend0:
	neumo_dvb_frontend_detach(dvb->frontend[0]);
...
static int dvb_init(struct cx231xx *dev)
{
	...
	for (i = 0; i < dev->board.adap_cnt; i++) {
		...
		dev->dvb[i] = dvb;
		snprintf(dev->card_address, sizeof(dev->card_address),
		...
		case CX231XX_BOARD_CNXT_RDE_250:
			...
			strscpy(dvb->frontend[0]->ops.info.card_address, dev->card_address,
		...
		case CX231XX_BOARD_ASTROMETA_T2HYBRID:
			{
				...
				dvb_attach(r820t_attach, dvb->frontend[0],
									 tuner_i2c,
									 &astrometa_t2hybrid_r820t_config, rf_in);
				break;
			}
		...
		case CX231XX_BOARD_TBS_5990:
			{
				...
				msleep(100);
				/* read mac */
				tbs_cx_mac(&dev->i2c_bus[1].i2c_adap, i, (u8*)&dvb->adapter_mac_address);
				dev_info(dev->dev, "TurboSight TBS5990 MAC[%i]: %pM\n", i, &dvb->adapter_mac_address);
				if(dev->card_mac_address == 0 )
					dev->card_mac_address = dvb->adapter_mac_address; //Select mac of first adapter as card mac_address

				dvb->frontend[0]->ops.info.num_rf_inputs =1;
				dvb->frontend[0]->ops.info.default_rf_input = i;
				dvb->frontend[0]->ops.info.rf_inputs[0] = i;

				dvb->frontend[0]->ops.info.card_mac_address = dev->card_mac_address;
				if(dev->board.short_name !=0) {
					strscpy(dvb->frontend[0]->ops.info.card_short_name, dev->board.short_name,
		...
		case CX231XX_BOARD_HAUPPAUGE_975:
			{
				...
				strscpy(dvb->frontend[1]->ops.info.card_address, dev->card_address,
								sizeof(dvb->frontend[1]->ops.info.card_address));
