...
struct em28xx_dvb {
	struct neumo_dvb_frontend        *fe[2];
	...
	int (*gate_ctrl)(struct neumo_dvb_frontend *fe, int gate);
	...
	for (i = 0; i < num_packets; i++) {
		if (xfer_bulk) {
			...
			if (!urb->actual_length)
				continue;
			neumo_dvb_dmx_swfilter(&dev->dvb->demux, urb->transfer_buffer,
					 urb->actual_length);
		} else {
			...
			if (!urb->iso_frame_desc[i].actual_length)
				continue;
			neumo_dvb_dmx_swfilter(&dev->dvb->demux,
					 urb->transfer_buffer +
...
static int em28xx_start_feed(struct neumo_dvb_demux_feed *feed)
{
...
static int em28xx_stop_feed(struct neumo_dvb_demux_feed *feed)
{
	struct neumo_dvb_demux *demux  = feed->demux;
	struct em28xx_dvb *dvb = demux->priv;
	int err = 0;
...
/* ------------------------------------------------------------------ */
static int em28xx_dvb_bus_ctrl(struct neumo_dvb_frontend *fe, int acquire)
{
...
static int drxk_gate_ctrl(struct neumo_dvb_frontend *fe, int enable)
{
...
static int em28xx_pctv_290e_set_lna(struct neumo_dvb_frontend *fe)
{
...
static int em28xx_pctv_292e_set_lna(struct neumo_dvb_frontend *fe)
{
	struct neumo_dtv_frontend_properties *c = &fe->dtv_property_cache;
	struct em28xx_i2c_bus *i2c_bus = fe->dvb->priv;
	struct em28xx *dev = i2c_bus->dev;
...
static int em28xx_mt352_terratec_xs_init(struct neumo_dvb_frontend *fe)
{
...
static noinline_for_stack int em28xx_attach_xc3028(u8 addr, struct em28xx *dev)
{
	...
	if (!fe) {
		...
		neumo_dvb_frontend_detach(dev->dvb->fe[0]);
		dev->dvb->fe[0] = NULL;
		return -EINVAL;
	}
			       ...
			       struct em28xx *dev, struct device *device)
{
	...
	/* register frontend */
	result = neumo_dvb_register_frontend(&dvb->adapter, dvb->fe[0]);
	if (result < 0) {
		dev_warn(&dev->intf->dev,
			 "neumo_dvb_register_frontend failed (errno = %d)\n",
			 result);
		goto fail_frontend0;
	}
	...
	if (dvb->fe[1]) {
		result = neumo_dvb_register_frontend(&dvb->adapter, dvb->fe[1]);
		if (result < 0) {
			dev_warn(&dev->intf->dev,
				 "2nd neumo_dvb_register_frontend failed (errno = %d)\n",
				 result);
			goto fail_frontend1;
		}
	}
	...
	if (result < 0) {
		dev_warn(&dev->intf->dev,
			 "neumo_dvb_dmx_init failed (errno = %d)\n",
			 result);
		goto fail_dmx;
	}
	...
	if (result < 0) {
		dev_warn(&dev->intf->dev,
			 "neumo_dvb_dmxdev_init failed (errno = %d)\n",
			 result);
		goto fail_dmxdev;
	}
...
fail_dmx:
	if (dvb->fe[1])
		neumo_dvb_unregister_frontend(dvb->fe[1]);
	neumo_dvb_unregister_frontend(dvb->fe[0]);
fail_frontend1:
	if (dvb->fe[1])
		neumo_dvb_frontend_detach(dvb->fe[1]);
fail_frontend0:
	neumo_dvb_frontend_detach(dvb->fe[0]);
...
static inline void prevent_sleep(struct neumo_dvb_frontend_ops *ops)
{
...
static int em28xx_dvb_suspend(struct em28xx *dev)
{
	...
	if (dev->dvb) {
		...
		if (dvb->fe[0]) {
			ret = neumo_dvb_frontend_suspend(dvb->fe[0]);
			dev_info(&dev->intf->dev, "fe0 suspend %d\n", ret);
		}
		if (dvb->fe[1]) {
			neumo_dvb_frontend_suspend(dvb->fe[1]);
...
static int em28xx_dvb_resume(struct em28xx *dev)
{
	...
	if (dev->dvb) {
		...
		if (dvb->fe[0]) {
			ret = neumo_dvb_frontend_resume(dvb->fe[0]);
			dev_info(&dev->intf->dev, "fe0 resume %d\n", ret);
		}

		if (dvb->fe[1]) {
			ret = neumo_dvb_frontend_resume(dvb->fe[1]);
