...
static int tbsci_i2c_read(struct tbsci_state *state)
{
	int ret;
	u8 buf = 0;
...
static int tbsci_i2c_write(struct tbsci_state *state,
...
int tbsci_init(struct saa716x_adapter *adap, int tbsci_nr, int tbsci_mode)
{
	...
	ret = neumo_dvb_ca_en50221_init(&adap->dvb_adapter, &state->ca,
		/* flags */ 0, /* n_slots */ 1);
