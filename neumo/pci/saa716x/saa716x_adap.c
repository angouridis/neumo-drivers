...
static int saa716x_dvb_start_feed(struct neumo_dvb_demux_feed *dvbdmxfeed)
{
...
static int saa716x_dvb_stop_feed(struct neumo_dvb_demux_feed *dvbdmxfeed)
{
	struct neumo_dvb_demux *dvbdmx		= dvbdmxfeed->demux;
	struct saa716x_adapter *saa716x_adap	= dvbdmx->priv;
	struct saa716x_dev *saa716x		= saa716x_adap->saa716x;
...
int saa716x_dvb_init(struct saa716x_dev *saa716x)
{
	...
	for (i = 0; i < config->adapters; i++) {
		...
		if ((result = neumo_dvb_dmx_init(&saa716x_adap->demux)) < 0) {
		...
		if ((result = neumo_dvb_dmxdev_init(&saa716x_adap->dmxdev,
...
//		tasklet_init(&saa716x_adap->tasklet, saa716x_dma_xfer, (unsigned long) saa716x);
		...
		if (config->frontend_attach) {
			...
			if (saa716x_adap->fe == NULL) {
				...
				result = neumo_dvb_register_frontend(&saa716x_adap->dvb_adapter, saa716x_adap->fe);
				if (result < 0) {
	...
	/* Error conditions */
err6:
	neumo_dvb_frontend_detach(saa716x_adap->fe);
err4:
...
err2:
	neumo_dvb_dmxdev_release(&saa716x_adap->dmxdev);
err1:
	neumo_dvb_dmx_release(&saa716x_adap->demux);
err0:
...
void saa716x_dvb_exit(struct saa716x_dev *saa716x)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (saa716x_adap->fe) {
			neumo_dvb_unregister_frontend(saa716x_adap->fe);
			neumo_dvb_frontend_detach(saa716x_adap->fe);
		}
