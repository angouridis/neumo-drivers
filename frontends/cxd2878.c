...
static int cxd2878_set_dvbc(struct neumo_dvb_frontend *fe)
{
	struct cxd2878_dev *dev = fe->demodulator_priv;
	int ret= 0;
...
static int cxd2878_set_atsc(struct neumo_dvb_frontend *fe)
{
	struct cxd2878_dev *dev = fe->demodulator_priv;
	int ret= 0;
...
static int cxd2878_read_status(struct neumo_dvb_frontend *fe,
...
static int cxd2878_tune(struct neumo_dvb_frontend*fe,bool re_tune,
...
static int cxd2878_set_property(struct neumo_dvb_frontend*fe,
		u32 cmd,u32 data)
{
	int ret = 0;
	switch(cmd){
...
static enum neumo_dvbfe_algo cxd2878_get_algo(struct neumo_dvb_frontend *fe)
{
...
static int cxd2878_read_ucblocks(struct neumo_dvb_frontend *fe,u32 *ucblocks)
{
	*ucblocks = 0;

	return 0;
}
static void cxd2878_spi_read(struct neumo_dvb_frontend *fe, struct ecp3_info *ecp3inf)
{
...
static void cxd2878_spi_write(struct neumo_dvb_frontend *fe,struct ecp3_info *ecp3inf)
{
...
static void cxd2878_eeprom_read(struct neumo_dvb_frontend *fe, struct eeprom_info *eepinf)
{
	struct cxd2878_dev *dev = fe->demodulator_priv;
...
static void cxd2878_eeprom_write(struct neumo_dvb_frontend *fe,struct eeprom_info *eepinf)
{
	struct cxd2878_dev *dev = fe->demodulator_priv;
...
static void cxd2878_release (struct neumo_dvb_frontend*fe)
{
	struct cxd2878_dev *dev = fe->demodulator_priv;
	dev->base->count--;
	if(dev->base->count == 0){
...
static const struct neumo_dvb_frontend_ops cxd2878_ops = {
...
struct neumo_dvb_frontend* cxd2878_attach(struct cxd2878_config* config,
					   struct i2c_adapter*i2c)
{
	...
	memcpy(&dev->fe.ops,&cxd2878_ops,sizeof(struct neumo_dvb_frontend_ops));
