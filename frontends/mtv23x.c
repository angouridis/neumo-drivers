...
static int mtv23x_init(struct neumo_dvb_frontend *fe)
{
...
static int mtv23x_set_frontend(struct neumo_dvb_frontend *fe)
{
	struct i2c_client*client = fe->demodulator_priv;
	struct mtv23x_dev*dev = i2c_get_clientdata(client);
...
static int mtv23x_read_status(struct neumo_dvb_frontend *fe, enum fe_status *status)
{
	struct i2c_client*client = fe->demodulator_priv;
	struct mtv23x_dev*dev = i2c_get_clientdata(client);
	struct neumo_dtv_frontend_properties *c = &fe->dtv_property_cache;
...
static int mtv23x_read_ber(struct neumo_dvb_frontend *fe,u32 *ber)
{
	struct i2c_client * client = fe->demodulator_priv;
	struct mtv23x_dev * dev = i2c_get_clientdata(client);
	struct neumo_dtv_frontend_properties *c = &fe->dtv_property_cache;
...
static struct neumo_dvb_frontend_ops mtv23x_ops = {
	.delsys = {SYS_ISDBT},
	.info 	= {
...
#endif
{
	...
	memcpy(&dev->fe.ops,&mtv23x_ops,sizeof(struct neumo_dvb_frontend_ops));
