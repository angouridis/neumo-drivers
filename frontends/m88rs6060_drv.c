...
#endif
#if 0
static int m88rs6060_sleep_(struct neumo_dvb_frontend *fe)
{
...
static int m88rs6060_wait_for_demod_lock_non_blind(struct m88rs6060_state* state)
{
	...
	for (i = 0; i < 150; i++) {
		if ((i% 20==19) &&  (kthread_should_stop() || neumo_dvb_frontend_task_should_stop(&state->fe))) {
...
static int m88rs6060_tune_once(struct neumo_dvb_frontend *fe, bool blind)
{
	struct m88rs6060_state* state = fe->demodulator_priv;
	struct i2c_client *client = state->demod_client;
	struct neumo_dtv_frontend_properties* p = &fe->dtv_property_cache;
 ...
 */
static int m88rs6060_read_status(struct neumo_dvb_frontend* fe, enum fe_status* status)
{
	struct m88rs6060_state* state = fe->demodulator_priv;
	struct i2c_client* client = state->demod_client;
	struct neumo_dtv_frontend_properties* p = &fe->dtv_property_cache;
...
static int m88rs6060_stop_task(struct neumo_dvb_frontend *fe)
{
	struct m88rs6060_state* state = fe->demodulator_priv;
...
static enum neumo_dvbfe_algo m88rs6060_get_algo(struct neumo_dvb_frontend *fe)
{
	//dprintk("%s()\n", __func__);
	return DVBFE_ALGO_HW;
}

static int m88rs6060_get_spectrum_scan_sweep(struct neumo_dvb_frontend* fe,
...
static int m88rs6060_spectrum_start(struct neumo_dvb_frontend* fe,
...
static int m88rs6060_spectrum_get(struct neumo_dvb_frontend* fe, struct dtv_fe_spectrum* user)
{
	struct m88rs6060_state* state = fe->demodulator_priv;
	int error=0;
...
static int m88rs6060_spectral_scan_start(struct neumo_dvb_frontend *fe, unsigned int *delay,  enum fe_status *status)
{
	struct m88rs6060_state* state = fe->demodulator_priv;
	struct spectrum_scan_state* ss = &state->spectrum_scan_state;
	//	struct neumo_dtv_frontend_properties *p = &fe->dtv_property_cache;
...
static int m88rs6060_spectral_scan_next(struct neumo_dvb_frontend *fe,
...
static int m88rs6060_scan_sat(struct neumo_dvb_frontend* fe, bool init,
														unsigned int *delay,  enum fe_status *status)
{
 	int error = -1;
	struct m88rs6060_state* state = fe->demodulator_priv;
...
void m88rs6060_detach(struct neumo_dvb_frontend* fe)
{
	struct m88rs6060_state* state = fe->demodulator_priv;
	struct i2c_client *client = state->demod_client;
...
//		goto err_regmap_0_regmap_exit;
	...
	memcpy(&state->fe.ops, &m88rs6060_ops, sizeof(struct neumo_dvb_frontend_ops));
